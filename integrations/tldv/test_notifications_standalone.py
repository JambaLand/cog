"""Testes do módulo de notificações (sem dependências do Cog)."""
...
class TestTeamsNotificationProvider:
    ...
    def test_validate_config_invalid_url(self) -> None:
        ...
        provider = TeamsNotificationProvider(webhook_url="http://example.com")

        with pytest.raises(ValueError, match="HTTPS"):
            provider.validate_config()

    def test_build_adaptive_card(
        self, teams_webhook: str, sample_message: dict
    ) -> None:
    ...
    def test_adaptive_card_contains_data(
        self, teams_webhook: str, sample_message: dict
    ) -> None:
        """Testar que Adaptive Card contém dados da mensagem."""
        provider = TeamsNotificationProvider(webhook_url=teams_webhook)
        card = provider._build_adaptive_card(sample_message)

        # Converter card para string para buscar conteúdo
        card_str = str(card)

        assert sample_message["title"] in card_str
        assert sample_message["summary"] in card_str
...
class TestGoogleCalendarNotificationProvider:
    ...
    def test_build_calendar_event(
        self, google_api_key: str, sample_message: dict
    ) -> None:
...
class TestNotificationManager:
    ...
    def test_register_provider(self, teams_webhook: str) -> None:
